"""Shared helper functions used by the maze generation algorithms."""
...
def init_protected(
    ...
    if height < 10 or width < 10:
        print_box((
            f"Pattern [{pattern}] cannot be"
            " contained within the maze! (10 x 10 is required)",
            "Warning",
            red,
        ))
        return set()
    ...
    if not pat or not pat[0]:
        print_box((
            f"Pattern [{pattern}] not recognized, "
            "skipping. (must be uppercase or number)",
            "Warning",
            red,
        ))
        return set()
