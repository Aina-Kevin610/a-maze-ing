"""ASCII terminal renderer for mazes, with an interactive menu."""
...
def cell_bg(
    ...
    """Return the ANSI background sequence for a single maze cell.
    ...
    Args:
        ...
        show_path: Whether the path should be highlighted.
        protected: Set of protected coordinates, if any.

    Returns:
        The colored two-space string to print for this cell.
    """
    if (x, y) == entry:
...
def parse(filename: str) -> tuple[
    ...
    """Parse a saved maze file into its grid, entry, exit and path.

    Args:
        filename: Path to the maze output file.

    Returns:
        A tuple (grid, entry, exit, path).
    """
...
def render(
    ...
    """Print the full ASCII representation of the maze to the terminal.
    ...
    Args:
        ...
        wall_color: ANSI background color used for the walls.
        show_path: Whether the path should be highlighted.
        protected: Set of protected coordinates, if any.
    """
    height = len(grid)
...
def ascii_render(
    ...
    """Run the interactive ASCII maze renderer loop.

    Displays the maze in the terminal and offers a menu to
    regenerate, toggle the path, change the wall color, or quit.

    Args:
        filename: Path to the maze output file to render.
        protected: Set of protected coordinates, if any.
        info: Optional lines of information to display above the maze.
    """
    show_path = True
    current_wall_key = "1"
